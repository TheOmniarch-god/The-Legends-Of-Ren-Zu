...
// Fast installed-app startup: cache-first app shell, stale-while-revalidate assets.

const CACHE_VERSION = "renzu-v2026-07-11-speed-1";
...
self.addEventListener("fetch", (event) => {
  ...
  // Never cache API calls; they must stay live.
  if (url.pathname.startsWith("/api/")) return;

  // Navigation/app startup: return cached shell immediately, refresh in background.
  ...
  // Static app assets: cache-first, refresh quietly.
  if (
  ...
  // CDN scripts/styles: stale-while-revalidate to reduce repeated launch delay.
